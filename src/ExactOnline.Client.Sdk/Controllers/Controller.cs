...
public class Controller<T> : IController<T>, IEntityManager where T : class
{
	...
	/// Returns the number of entities of the current type
	/// </summary>
	public Task<int> CountAsync(string query, CancellationToken ct = default) => _conn.CountAsync(query, ct);
	...
	public async Task<Models.ApiList<T>> GetAsync(string query, EndpointTypeEnum endpointType, CancellationToken ct = default)
	{
		...
		// Get the response and convert it to a list of entities of the specific type
		var response = await _conn.GetAsync(query, endpointType, ct).ConfigureAwait(false);
	...
	public async Task<T> CreateAsync(T entity, CancellationToken ct = default)
	{
		...
		// Send to API
		var response = await _conn.PostAsync(json, ct).ConfigureAwait(false);
		if (!response.Contains("error"))
		{
			// Set values of API in account entity (to ensure GUID is set)
			response = ApiResponseCleaner.GetJsonObject(response);
